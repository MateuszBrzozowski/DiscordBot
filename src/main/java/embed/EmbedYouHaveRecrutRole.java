...
public class EmbedYouHaveRecrutRole {
    private RangerLogger rangerLogger = new RangerLogger();

    public EmbedYouHaveRecrutRole(ButtonClickEvent event) {
        event.getJDA().retrieveUserById(event.getUser().getId()).queue(user -> {
            user.openPrivateChannel().queue(privateChannel -> {
                ...
                rangerLogger.info("Użytkonik ["+user.getName()+"] chciał złożyć podanie. Ma przypisaną rolę rekruta.");
            });
        });
    }
}
